...
public class GeocodingApiTest {
	...
	public void testGeocode() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(simpleGeocodeResponse)) {
			GeocodingResult[] results = GeocodingApi.geocode(sc.context, "Sydney").await();
			checkSydneyResult(results);
	...
	public void testReverseGeocoding() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext(simpleGeocodeResponse)) {
			LatLng sydney = new LatLng(-33.8674869, 151.2069902);
			GeocodingResult[] results = GeocodingApi.reverseGeocode(sc.context, sydney).await();
			checkSydneyResult(results);
		}
	}
	...
	/**
	 * Address geocode with bounds: <a href=
	...
	/**
	 * Geocode with region biasing: <a href=
	 * "https://maps.googleapis.com/maps/api/geocode/json?address=Toledo&region=es">Geocode
	...
	/**
	 * Geocode with component filtering: <a href=
	...
	/**
	 * Geocode with multiple component filters: <a href=
	 * "https://maps.googleapis.com/maps/api/geocode/json?address=Torun&components=administrative_area:TX|country:US">
	 * Geocoding Torun, with administrative area of "TX" and country of
	 * "US"</a>.
	 */
	@Test
	...
	/**
	 * Making a request using just components filter: <a href=
	 * "https://maps.googleapis.com/maps/api/geocode/json?components=route:Annegatan|administrative_area:Helsinki|country:Finland">
	 * Searching for a route of Annegatan, in the administrative area of
	 * Helsinki, and the country of Finland </a>.
	 */
	@Test
	public void testGeocodeWithJustComponents() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext("\n" + "{\n" + "   \"results\" : [\n" + "      {\n"
			...
			GeocodingResult[] results = GeocodingApi.newRequest(sc.context)
					.components(ComponentFilter.route("Annegatan"), ComponentFilter.administrativeArea("Helsinki"),
	...
	/**
	 * Simple reverse geocoding. <a href=
	 * "https://maps.googleapis.com/maps/api/geocode/json?latlng=40.714224,-73.961452">Reverse
	...
	/**
	 * Reverse geocode restricted by type: <a href=
	 * "https://maps.googleapis.com/maps/api/geocode/json?latlng=40.714224,-73.961452&location_type=ROOFTOP&result_type=street_address">
	 * Reverse Geocode (40.714224,-73.961452) with location type of ROOFTOP and
	 * result type of street_address</a>.
	 */
	@Test
	public void testReverseGeocodeRestrictedByType() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext("\n" + "{\n" + "   \"results\" : [\n" + "      {\n"
				...
				+ "}\n")) {
			LatLng latlng = new LatLng(40.714224, -73.961452);
			GeocodingResult[] results = GeocodingApi.newRequest(sc.context).latlng(latlng)
	 ...
	 * <p>
	 * See <a href=
	 * "https://googlegeodevelopers.blogspot.com.au/2016/11/address-geocoding-in-google-maps-apis.html">
	 * Address Geocoding in the Google Maps APIs</a> for the reasoning behind
	 * this usage.
	 */
	@Test
	public void testCustomParameterPassThrough() throws Exception {
		try (LocalTestServerContext sc = new LocalTestServerContext("\n" + "{\n" + "   \"results\" : [\n" + "      {\n"
				...
				+ "}\n")) {
			String address = "1600 Amphitheatre Parkway, Mountain View, CA";
			GeocodingResult[] results = GeocodingApi.newRequest(sc.context).address(address)
					.custom("new_forward_geocoder", "true").await();
